...
module Vault
  ...
  class Logical < Request
    ...
    def list(path, options = {})
      ...
      json = client.list("/v1/#{CGI.escape(full_path(path))}", {}, headers)
      json[:data][:keys] || []
    rescue HTTPError => e
    ...
    def read(path, options = {})
      headers = extract_headers!(options)
      json = client.get("/v1/#{CGI.escape(full_path(path, options))}", {}, headers)
    ...
    def write(path, data = {}, options = {})
      headers = extract_headers!(options)
      json = client.put("/v1/#{CGI.escape(full_path(path))}", JSON.fast_generate(data), headers)
      if json.nil?
        return true
      else
        return Secret.decode(json)
